...
Now, we want to add the old annotations from the storage account. 
And re-run this whole process to re-create the metadata_for_map. 
Let's get the id's of the old blurring annotations

Then, let's see if we have duplicates with the new annotations.
We have placed the old-blurring-dataset with images and label inside the blurring-project folder
...
paths_old_blur = [path for path in paths if path.startswith("/old-blurring-dataset/images/")]
pano_ids_old_blur = [os.path.splitext(os.path.basename(path))[0] for path in paths_old_blur]

print(f"{len(pano_ids_old_blur)} images in the old blurring dataset.")

with open('../data-prep/diana/in:coco-format/blur_v0.1/old-blurring-ids.json', 'w') as outfile:
    json.dump(pano_ids_old_blur, outfile)
```
We have 548 images in the old blurring dataset, which we store in a json, next to the blur-annotation.json
...
The output is: 
```text
There is a total of 1994 images, 1446 new and 548 old.
There is a total of 1993 unique images.
```

This means that the old and new set do not overlap. 
The only duplicate comes from the new set.
...
After adding 548 points of interest, we have 1994 points of interest.
```

Next, we want to put all images tha t have annotations in a separate dataset
Namely, we take file_names from the metadata_for_map.json
and we download all these images from the blurring-project blob
...
panorama_ids = [point.pano_id for point in points]
...
We downloaded the 1993 images (we have one duplicate). 
Previously, we generated the new annotations txt files.
Now we will move them to blur_v0.1/all/labels and combine with the old annotations txt files.

We found some issues with the old_blurring_dataset, there are ~3500 txt files but only ~500 images.
